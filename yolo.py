...
def draw_prediction(img, class_id, confidence, x, y, x_plus_w, y_plus_h, classes, size):
...
def find_vehicles(image, tiny):
    ...
    for i in indices:
        ...
        #cv2.waitKey()
        draw_prediction(image, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h), classes, sizes[i])
